export default function PreferenceCard({ user, onEdit }) {
  const prefs = [
    { icon: 'Lang',     label: 'Preferred Language', value: user?.preferredLanguage },
    { icon: 'Crop',     label: 'Primary Crop',        value: user?.primaryCrop },
    { icon: 'Location', label: 'Location',            value: [user?.district, user?.state].filter(Boolean).join(', ') || null },
    { icon: 'Farm',     label: 'Farm Size',           value: user?.farmSize > 0 ? ` acres` : null },
  ]
