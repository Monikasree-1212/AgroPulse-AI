...
export default function ProfitSimulator() {
  ...
  const handleSubmit = async (e) => {
    ...
    setResult(null)
    try {
      const { data } = await api.post('/api/profit/simulate', form)
      setResult(data)
      // Auto-create notification for HOLD/SELL recommendation
      try {
        await api.post('/api/notifications', {
          title: data.recommendation === 'HOLD'
            ? `Hold ${form.commodity} for Higher Profit Price`
            : `Sell ${form.commodity} Today `,
          message: data.recommendation === 'HOLD'
            ? `Profit Simulator recommends HOLD. Expected gain: Rs.${data.profitDifference.toLocaleString()} (ROI: +${data.roi}%) after ${form.daysToHold} days.`
            : `Profit Simulator recommends SELL TODAY. Holding may result in Rs.${Math.abs(data.profitDifference).toLocaleString()} loss (ROI: ${data.roi}%).`,
          type: 'profit',
  ...
  const kpis = result ? [
    {
      icon: 'Revenue', label: 'Current Revenue',   value: `Rs.${result.currentRevenue.toLocaleString()}`,
      sub: `${result.quantity} kg  Rs.${form.currentPrice}`,
    ...
    {
      icon: 'Revenue', label: 'Future Revenue',    value: `Rs.${result.futureRevenue.toLocaleString()}`,
      sub: `${result.quantity} kg  Rs.${form.predictedPrice}`,
    ...
    {
      icon: 'Storage', label: 'Storage Expense',   value: `Rs.${result.storageExpense.toLocaleString()}`,
      sub: `${result.quantity} kg  Rs.${form.storageCost}`,
    ...
    {
      icon: 'Transport', label: 'Transport Expense', value: `Rs.${result.transportExpense.toLocaleString()}`,
      sub: `${result.quantity} kg  Rs.${form.transportCost}`,
    ...
    {
      icon: result.profitDifference >= 0 ? '?' : 'Downward',
      label: 'Profit Difference',
      value: `${result.profitDifference >= 0 ? '+' : ''}Rs.${result.profitDifference.toLocaleString()}`,
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Field label="Current Price Rs." name="currentPrice"   value={form.currentPrice}   onChange={handleChange} />
            <Field label="Predicted Price Rs." name="predictedPrice" value={form.predictedPrice} onChange={handleChange} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="Storage Cost Rs./kg"   name="storageCost"   value={form.storageCost}   onChange={handleChange} />
            <Field label="Transport Cost Rs./kg" name="transportCost" value={form.transportCost} onChange={handleChange} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center text-xl">Balance</div>
                <div>
              ...
              <p className={`text-xs font-semibold mt-1.5 ${form.predictedPrice >= result.breakEvenPrice ? 'text-green-500' : 'text-red-500'}`}>
                {form.predictedPrice >= result.breakEvenPrice
                  ? `Yes Predicted price Rs.${form.predictedPrice} exceeds break-even`
                  : `No Predicted price Rs.${form.predictedPrice} is below break-even`}
              </p>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} tickFormatter={(v) => `Rs.${(v / 1000).toFixed(0)}k`} />
